...
if __name__ == '__main__':
    # pd config
    pdConfig()
    keyword_path = 'C:/Users/minkun/OneDrive/minkun/pyCharmWP/productNameModifier/main/resources/상품명가공 키워드지우기 리스트.xlsx'
    # \하나는 인식못한다. /를 쓰거나 \\ 2개 써야 함. 밑에 함수떄문에 /를 써야한다.
    path = "resources/최저가확인관 인기순.xls"
    ...
    # df 전처리
    df.drop(inplace=True, index=0, axis=0)  # 행삭제. 행을 삭제해도 인덱스는 그대로 남아있다.
    ...
    file_name , extension = core.get_file_name(path, sep='/')
    extension='xlsx'
    cnt = 1
    dir_name = 'resources'
    dir_name1 = 'resXls'
