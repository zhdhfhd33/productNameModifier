...
def get_file_name(path, sep):
    ...
    :param sep: '/' or '\\'
    :return: file_name(=최저가확인관 최근순), extension(=.xlsx)
    """
    li = path.split(sep=sep)
    full_name = li[len(li) - 1]
    extension_idx = full_name.rfind('.')

    extension = full_name[extension_idx:]
...
def get_img_mask(img, R, G, B):
    ...
    notMatch = 0
    mask = [[match if img[j][i][0] == R and img[j][i][1] == G and img[j][i][2] == B else notMatch for i in range(M)] for
            j in range(N)]  # 2차원 배열 만들기. 특이하게 col을 먼저 써야한다.
...
def make_cnt_path(file_path):
    """
    :param file_path: 이미 존재하는 파일일 경우 _1, _2 이런 식으로 파일 이름을 변경한 파일이름 생성.
    :return:
    """
    cnt = 0
    ...
    while os.path.isfile(file_path):  # 여러번 실행시킬 수 있도록.
        cnt += 1
        file_path = base + '_' + str(cnt) + ext # file_path 갱신.
    return file_path
